...
class Router {
  ...
  async queue (res, req, def, obj, in_, out, params, queue) {
    this.log(req)
    const card = parseInt(req.getParameter(0))
    const error = checkCard(card, def, obj, in_, out, queue)
    if (error !== undefined) {
  ...
  run (def, obj, prefix) {
    ...
    this.app.get(prefix + '/queue/exit/in/:card', async (res, req) => {
      ...
      await this.queue(res, req, def, obj, true, false, def.REQ_EXIT_IN, obj.exitQueue)
    })
    this.app.get(prefix + '/queue/exit/out/:card', async (res, req) => {
      ...
      await this.queue(res, req, def, obj, false, true, def.REQ_EXIT_OUT, obj.exitQueue)
    })
    this.app.get(prefix + '/queue/swap/ev/in/:card', async (res, req) => {
      ...
      await this.queue(res, req, def, obj, true, false, def.REQ_SWAP_IN, obj.swapQueue)
    })
    this.app.get(prefix + '/queue/swap/ev/out/:card', async (res, req) => {
      ...
      await this.queue(res, req, def, obj, false, true, def.REQ_SWAP_OUT, obj.swapQueue)
    })
