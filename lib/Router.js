...
class Router {
  ...
  // Exits requests (EV and standard users)
  async exit (res, req, def, obj, in_, out, ev, st, params, queue) {
    this.log(req)
  ...
  // Swap requests (EV users only)
  async swap (res, req, def, obj, in_, out, ev, st, params, queue) {
    this.log(req)
    const card = parseInt(req.getParameter(0))
    if (!Number.isInteger(card)) {
      return sendJson(res, this.error(card, CARD_NOT_VALID))
  ...
  run (def, obj, prefix) {
    ...
    this.app.get(prefix + '/queue/exit/:card', async (res, req) => {
      this.log(req)
      const card = parseInt(req.getParameter(0))
      if (!Number.isInteger(card)) {
        return sendJson(res, this.error(card, CARD_NOT_VALID))
      ...
      if (!obj.stalls.some(element => element.status === card)) {
        return sendJson(res, this.error(card, CARD_NOT_PARKED))
      }
      const { exitEvQueue: evQueue, exitStQueue: stQueue } = obj
      const ev = obj.cards.find(element => element.nr === card).type !== 0
      if (ev) {
        if (evQueue[evQueue.length - 1].card !== 0) {
          return sendJson(res, this.error(card, QUEUE_FULL))
        }
        if (evQueue.some(element => element.card === card)) {
          return sendJson(res, this.error(card, CARD_QUEUED))
        }
      } else {
        if (stQueue[stQueue.length - 1].card !== 0) {
          return sendJson(res, this.error(card, QUEUE_FULL))
        }
        if (stQueue.some(element => element.card === card)) {
          return sendJson(res, this.error(card, CARD_QUEUED))
        }
      }
      res.onAborted(() => {
    ...
    this.app.get(prefix + '/queue/exit/ev/out/:card', async (res, req) => {
      await this.exit(res, req, def, obj, false, true, false, false, def.REQ_EXIT_EV_OUT, obj.exitEvQueue)
    })
    this.app.get(prefix + '/queue/exit/st/out/:card', async (res, req) => {
      await this.exit(res, req, def, obj, false, true, false, false, def.REQ_EXIT_ST_OUT, obj.exitStQueue)
    })
    this.app.get(prefix + '/queue/swap/ev/in/:card', async (res, req) => {
      await this.swap(res, req, def, obj, true, false, true, false, def.REQ_SWAP_EV_IN, obj.swapEvQueue)
    })
    this.app.get(prefix + '/queue/swap/ev/out/:card', async (res, req) => {
      await this.swap(res, req, def, obj, false, true, true, false, def.REQ_SWAP_EV_OUT, obj.swapEvQueue)
    })
    this.app.get(prefix + '/queue/swap/st/in/:card', async (res, req) => {
      await this.swap(res, req, def, obj, true, false, false, true, def.REQ_SWAP_ST_IN, obj.swapStQueue)
    })
    this.app.get(prefix + '/queue/swap/st/out/:card', async (res, req) => {
      await this.swap(res, req, def, obj, false, true, false, true, def.REQ_SWAP_ST_OUT, obj.swapStQueue)
    })
