...
class Plc extends EventEmitter {
  ...
  data (def, obj) {
    try {
      setTimeout(async () => {
        if (this.online) {
          ...
          await Promise.all([
            ...
            updateQueue(def.DB_DATA_INIT_EXIT_ST_QUEUE, buffer, 6, obj.exitStQueue),
            updateQueue(def.DB_DATA_INIT_EXIT_EV_QUEUE, buffer, 6, obj.exitEvQueue),
      ...
      const queue = obj.exitEvQueue.concat(obj.swapEvQueue).filter(item => item.card > 0 && obj.stalls.some(stall => stall.nr === item.slot && stall.ev_type !== 0))
      if (queue.length) {
        obj.q.index < queue.length - 1 ? obj.q.index += 1 : obj.q.index = 0
  ...
  async ev_stalls (def, obj) {
    try {
      ...
      obj.stalls.forEach(element => {
        ...
        element.ev_isCharging = buffer[byte + 2] & 1 ? 1 : 0 // bit .0
        element.ev_isHolstered = buffer[byte + 2] & 2 ? 1 : 0 // bit .1
        byte += 4
      })
    } catch (e) {
      this.error(e)
  ...
  async ev_unlock (def, slot) {
    try {
      ...
      // const amount = 2
      // const wordLen = 0x02
      const buffer = Buffer.alloc(1, 0) // set bit to False
      const start = slot === 1 ? (0 + 2) * 8 + 0 : (((slot - 1) * 4) + 2) * 8 + 0 // bit .0 is_charging
      const amount = 1
      const wordLen = 0x01 // Bit
      const response = await WriteArea(this.client, area, dbNumber, start, amount, wordLen, buffer)
