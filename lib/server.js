...
class Document {
  ...
  setEndpoint (n) {
    if (n === 1) return '/queue/exit/:card'
    if (n === 2) return '/queue/exit/ev/out/:card'
    if (n === 3) return '/queue/exit/st/out/:card'
    if (n === 4) return '/queue/swap/ev/in/:card'
    if (n === 5) return '/queue/swap/ev/out/:card'
    if (n === 6) return '/queue/swap/st/in/:card'
    if (n === 7) return '/queue/swap/st/out/:card'
    return undefined
  }
...
const server = (db, host, port) => {
  ...
  server.on('connection', function (sock) {
    ...
    sock.on('data', async function (data) {
      ...
      if (doc.endpoint !== undefined) insert.run(Object.values(doc))
